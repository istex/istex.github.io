...
$(self.elt).empty();$(self.elt).css({opacity:1.0});$(self.elt).append(facets);facets.fadeIn();};Plugin.prototype.getCorpusFacetDom=function(results,istexSearch){var self=this;if(!results||!results.aggregations||!results.aggregations.corpusName||!results.aggregations.corpusName.buckets){return null;}
var facetCorpus=$('<div class="istex-facet">'+'<h4 class="istex-facet-name">'+self.settings.labels.facets.corpus+'</h4>'+'<ul class="istex-facet-corpus"></ul>'+'</div>');results.aggregations.corpusName.buckets.forEach(function(corpus){var corpusCheckbox=$('<li>'+'<label>'+'<input type="checkbox" value="'+corpus.key+'" />'+
corpus.key+'<span class="istex-facet-corpus-badge">'+corpus.docCount+'</span>'+'</label>'+'</li>');if(self.selectedCorpus.indexOf(corpus.key)!==-1){corpusCheckbox.find('input').attr('checked','checked');}else{corpusCheckbox.find('input').removeAttr('checked');}
