...
$(self.elt).append('<button class="istex-ezproxy-auth-btn">Se connecter<div></div></button>');$(self.elt).find('.istex-ezproxy-auth-btn').click(cb);};Plugin.prototype.removeConnectBtn=function(cb){var self=this;$(self.elt).find('.istex-ezproxy-auth-btn').remove();};Plugin.prototype.authWithHTTP=function(cb){var self=this;self.insertConnectBtnIfNotExists(function(){if($(self.elt).find('.istex-auth-popup').length>0){return;}
$(self.elt).append('<form class="istex-auth-popup">'+'<div class="istex-auth-popup-wrapper">'+'<input class="istex-auth-popup-login" type="text" value="" placeholder="Votre login ..." />'+'<input class="istex-auth-popup-password" type="password" value="" placeholder="Votre mot de passe ..." />'+'<input class="istex-auth-popup-cancel" type="submit" value="Annuler" />'+'<input class="istex-auth-popup-submit" type="submit" value="Se connecter" />'+'</div>'+'<p class="istex-auth-popup-error"></p>'+'</form>');$(self.elt).find('.istex-auth-popup').submit(function(){var clicked=$(self.elt).find(".istex-auth-popup input[type=submit]:focus").attr('class');if(clicked=='istex-auth-popup-submit'){var httpOptions={headers:{"Authorization":"Basic "+btoa($(self.elt).find('.istex-auth-popup-login').val()+":"+
$(self.elt).find('.istex-auth-popup-password').val())}};$.ajax({url:self.settings.istexApi+'/corpus/',headers:httpOptions.headers,success:function(){$(self.elt).find('.istex-auth-popup').remove();return cb(null,httpOptions);},error:function(opt,err){$(self.elt).find('.istex-auth-popup-error').text("Le nom d'utilisateur ou le mot de passe saisi est incorrect.");}});}else if(clicked=='istex-auth-popup-cancel'){$(self.elt).find('.istex-auth-popup').remove();cb(new Error('HTTP auth canceled'));}
return false;});});};Plugin.prototype.setupGenericRequester=function(authMode,authOptions){var self=this;if(authMode=='jsonp'){self.istexApiRequester=function(options){var reqOpt=$.extend({callbackParameter:"callback"},authOptions,options);return $.jsonp(reqOpt);};}else{self.istexApiRequester=function(options){var reqOpt=$.extend(authOptions,options);return $.ajax(reqOpt);};}};$.fn[pluginName]=function(options){this.each(function(){if(!$.data(this,"plugin_"+pluginName)){$.data(this,"plugin_"+pluginName,new Plugin(this,options));}});return this;};})(jQuery,window,document);